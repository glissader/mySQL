...
(
    ...
    table_pk   bigint unsigned not null,
    name       varchar(255)
) engine Archive;

drop trigger if exists catalogsArchive;
create trigger catalogsArchive
    after insert
    on catalogs
    for each row
begin
    insert into logs(timestamp, table_name, table_pk, name)
    values (NOW(), 'catalogs', NEW.id, NEW.name);
end;

drop trigger if exists usersArchive;
create trigger usersArchive
    after insert
    on users
    for each row
begin
...
drop trigger if exists productsArchive;
create trigger productsArchive
    after insert
    on products
    for each row
begin
